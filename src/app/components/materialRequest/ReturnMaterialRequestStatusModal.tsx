...
export function ReturnMaterialRequestStatusModal({request,onClose,onReturned}:{request:MaterialRequest;onClose:()=>void;onReturned:()=>void}){
...
async function submit(){setError("");setLoading(true);try{await returnMaterialRequestStatusUseCase({requestId:request.id??0,targetStatus:targetStatus,reason,performedByName:"Administrador"});onReturned();}catch(e){setError(e instanceof Error?e.message:"Erro ao voltar status.");}finally{setLoading(false);}}
